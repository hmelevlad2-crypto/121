...
async function renderAboutPhoto() {
    ...
    // About photo upload
    adminAboutBtn.addEventListener('click', async () => {
        const file = adminAboutInput.files[0];
        if (!file) return alert(currentLang === 'ru' ? 'Выберите файл' : 'Datei auswählen');
        const data = await fileToBase64(file);
        await setAboutPhoto({ data, alt_ru: 'Фото художника', alt_de: 'Foto der Künstlerin' });
        adminAboutInput.value = '';
        await renderAboutPhoto();
        alert(currentLang === 'ru' ? 'Фото загружено!' : 'Foto hochgeladen!');
    });

    // About photo delete
    adminAboutDel.addEventListener('click', async () => {
        await deleteAboutPhoto();
        await renderAboutPhoto();
        alert(currentLang === 'ru' ? 'Фото удалено' : 'Foto gelöscht');
    });

    // Gallery upload
    adminGalleryBtn.addEventListener('click', async () => {
        const file = adminGalleryInput.files[0];
        if (!file) return alert(currentLang === 'ru' ? 'Выберите файл' : 'Datei auswählen');
        const data = await fileToBase64(file);
        ...
        const newItem = {
            ...
            data,
            wide: adminGalleryWide.checked,
            alt_ru: 'Работа',
            alt_de: 'Werk'
        };
        items.push(newItem);
        await setGallery(items);
        adminGalleryInput.value = '';
        adminGalleryWide.checked = false;
        await renderGallery();
    ...
    async function renderAdminGalleryList() {
        ...
        const t = translations[currentLang];
        if (items.length === 0) {
            adminGalleryList.innerHTML = `<p class="admin-empty">${t.admin_empty}</p>`;
            return;
        }
        adminGalleryList.innerHTML = items.map(item => `
            <div class="admin-gallery-item">
                ...
                <div class="admin-gallery-meta">
                    <span>${item.wide ? (currentLang === 'ru' ? 'Широкое' : 'Breit') : (currentLang === 'ru' ? 'Обычное' : 'Normal')}</span>
                    <button class="admin-gallery-del" data-id="${item.id}">${t.admin_delete}</button>
                </div>
            </div>
        `).join('');

        adminGalleryList.querySelectorAll('.admin-gallery-del').forEach(btn => {
